...
static ADS1299_ReturnType ADS1299_WriteRegs(uint8_t board, uint8_t address, uint8_t *pData, uint16_t size)
{
    ...
    ADS1299_pGlbCfg->pHw->Spi_TransmitReceive(&au8TxData[0], &au8RxData[0], size+2);
...
static ADS1299_ReturnType ADS1299_ReadRegs(uint8_t board, uint8_t address, uint8_t *pData, uint16_t size)
{
    ...
    ADS1299_pGlbCfg->pHw->Spi_TransmitReceive(au8TxData, au8RxData, size+2);
...
static ADS1299_ReturnType ADS1299_SendCmd(uint8_t board, uint8_t cmd)
{
	...
	ADS1299_pGlbCfg->pHw->Spi_TransmitReceive(&u8TxData, &u8RxData, 1);
...
ADS1299_ReturnType ADS1299_GetSensorData(uint8_t board)
{
	...
	ADS1299_pGlbCfg->pHw->Spi_TransmitReceive(NULL, &ADS1299_EEGRawDataBuffer[board][0], 3 + ADS1299_NUMBER_OF_CHANNEL * ADS1299_BYTEDATA_RESOLUTION);
...
#ifdef ADS1299_USING_SPI_DMA
// ADS1299_ReturnType ADS1299_GetSensorData_DMA(uint8_t board)
// {
// 	/* Check Parameters */
// 	if(ADS1299_NUMBER_OF_USED_BOARD <= board)
// 	{
...
// 	ADS1299_HwSpi.set_dma_usage(DMA_USAGE_ALWAYS);
// 	// ADS1299_SpiDmaCallBack.attach(&ADS1299_OnSpiDmaRxComplete)
// 	ADS1299_HwSpi.transfer(NULL, 0, \
// 	                       &ADS1299_SensorRawDataBuffer[board][0], 3 + ADS1299_NUMBER_OF_CHANNEL * ADS1299_BYTEDATA_RESOLUTION, \
// 	                       event_callback_t(this, &ADS1299_OnSpiDmaRxComplete),int event=SPI_EVENT_COMPLETE);
// 	event_callback_t

// }

// static void ADS1299_OnSpiDmaRxComplete(int event)
// {
//    // Do something
// }
#endif
...
ADS1299_ReturnType ADS1299_Init(void)
{
	...
	for(uint8_t u8BoardCnt = 0; u8BoardCnt < ADS1299_NUMBER_OF_USED_BOARD; u8BoardCnt++)
	{
		...
		{
			// ADS1299_DEBUG_PRINT("[ADS1299_ERR] Fail to read ID\r\n");
			return ADS1299_NOT_OK;
		}
